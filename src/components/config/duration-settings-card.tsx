...
export const DurationSettingsCard = forwardRef<DurationSettingsCardRef, DurationSettingsCardProps>((props, ref) => {
  ...
  const handleInputChange = (setter: React.Dispatch<React.SetStateAction<string>>, value: string, key: keyof FormatAndTimingsProfileData, isSeconds?: boolean) => {
    setter(value);
    if(onSettingsChange) {
      const numValue = parseInt(value, 10);
      if (!isNaN(numValue)) {
        onSettingsChange({ ...tempSettings, [key]: isSeconds ? numValue * 100 : numValue * 60 * 100 });
      }
    }
    markDirty();
  }
  ...
  const inputGrid = (
    <div className="grid grid-cols-[auto_theme(spacing.24)_auto_auto] items-center gap-x-3 sm:gap-x-4 gap-y-6">
      ...
      <Input id="numRegularPeriods" type="number" value={localNumRegularPeriodsInput} onChange={(e) => handleInputChange(setLocalNumRegularPeriodsInput, e.target.value, 'numberOfRegularPeriods')} className={cn(narrowInputStyle)} placeholder="ej. 3" min="1"/>
      <Label htmlFor="periodDuration" className="text-sm whitespace-nowrap justify-self-end">Duración (Min)</Label>
      <Input id="periodDuration" type="number" value={localPeriodDurationInput} onChange={(e) => handleInputChange(setLocalPeriodDurationInput, e.target.value, 'defaultPeriodDuration')} className={cn(narrowInputStyle)} placeholder="ej. 20" min="1"/>

      <Label htmlFor="numOTPeriods" className="text-sm whitespace-nowrap">Períodos Overtime (Cant)</Label>
      <Input id="numOTPeriods" type="number" value={localNumOTPeriodsInput} onChange={(e) => handleInputChange(setLocalNumOTPeriodsInput, e.target.value, 'numberOfOvertimePeriods')} className={cn(narrowInputStyle)} placeholder="ej. 1" min="0"/>
      <Label htmlFor="otPeriodDuration" className="text-sm whitespace-nowrap justify-self-end">Duración (Min)</Label>
      <Input id="otPeriodDuration" type="number" value={localOTPeriodDurationInput} onChange={(e) => handleInputChange(setLocalOTPeriodDurationInput, e.target.value, 'defaultOTPeriodDuration')} className={cn(narrowInputStyle)} placeholder="ej. 5" min="1"/>
      
      <Label htmlFor="timeoutDurationConfig" className="text-sm whitespace-nowrap">Timeout (seg)</Label>
      <Input id="timeoutDurationConfig" type="number" value={localTimeoutDurationInput} onChange={(e) => handleInputChange(setLocalTimeoutDurationInput, e.target.value, 'defaultTimeoutDuration', true)} className={cn(narrowInputStyle)} placeholder="ej. 30" min="1"/>
      ...
      <Input id="breakDurationConfig" type="number" value={localBreakDurationInput} onChange={(e) => handleInputChange(setLocalBreakDurationInput, e.target.value, 'defaultBreakDuration', true)} className={cn(narrowInputStyle)} placeholder="ej. 120" min="1"/>
      ...
      <Input id="preOTBreakDurationConfig" type="number" value={localPreOTBreakDurationInput} onChange={(e) => handleInputChange(setLocalPreOTBreakDurationInput, e.target.value, 'defaultPreOTBreakDuration', true)} className={cn(narrowInputStyle)} placeholder="ej. 60" min="1"/>
      ...
      <Input id="warmUpDurationConfig" type="number" value={localWarmUpDurationInput} onChange={(e) => handleInputChange(setLocalWarmUpDurationInput, e.target.value, 'defaultWarmUpDuration')} className={cn(narrowInputStyle)} placeholder="ej. 5" min="1"/>
