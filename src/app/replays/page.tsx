...
import { Video, ListVideo, Loader2, AlertTriangle, RefreshCw } from 'lucide-react';
...
export default function ReplaysPage() {
    ...
    // When selectedReplay changes, update the video src and try to play
    useEffect(() => {
        if (selectedReplay && videoRef.current) {
            videoRef.current.load(); // This is important to apply the new src
        }
    }, [selectedReplay]);

    return (
        <div className="flex flex-col h-[calc(100vh-8rem)] w-full max-w-7xl mx-auto py-8">
            ...
            <div className="flex-grow grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6 overflow-hidden">
                ...
                <Card className="col-span-1 md:col-span-2 lg:col-span-3 flex flex-col items-center justify-center bg-black">
                     {selectedReplay ? (
                        <video
                            ref={videoRef}
                            key={selectedReplay} // Force re-mount on src change
